...
import { ArrowUpDown, ChevronLeft, ChevronRight, Filter, Pencil, Plus, Search, Trash2, UserRound, X } from 'lucide-react';
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [result, filterOpts] = await Promise.all([
        fetchEmployees({ page, limit, search, country: countryFilter, job_title: jobFilter, sort_by: sortBy, sort_order: sortOrder }),
        fetchFilters(),
      ]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-5 rounded-lg border border-slate-200 bg-white px-5 py-5 shadow-sm md:flex-row md:items-start md:justify-between md:px-6">
        <div className="max-w-2xl">
          ...
          <h1 className="text-xl font-bold text-slate-950 md:text-2xl">Employees</h1>
          <p className="mt-2 max-w-none text-sm leading-6 text-slate-500 xl:whitespace-nowrap">
            Manage salary records, filter across countries and roles, and keep compensation data current.
          </p>
        </div>
        <button
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(220px,1fr)_minmax(150px,190px)_minmax(180px,250px)_auto] lg:items-end">
          ...
          <label className="space-y-1.5">
            ...
            <FilterSelect
              ...
              value={jobFilter}
              onChange={(value) => { setJobFilter(value); setPage(1); }}
              options={[
          ...
          <button
            onClick={() => { setSearch(''); setCountryFilter(''); setJobFilter(''); setPage(1); }}
            disabled={!hasFilters}
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-2 border-b border-slate-200 bg-slate-50/80 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">Showing {firstVisible.toLocaleString()} - {lastVisible.toLocaleString()} of {count.toLocaleString()}</p>
          </div>
                ...
                {columns.map(col => (
                  ...
                  >
                    <div className="flex items-center gap-1.5">
                      {col.label}
                      <ArrowUpDown className={`h-3.5 w-3.5 ${sortBy === col.key ? 'text-emerald-700' : 'text-slate-300'}`} />
                    </div>
                  </th>
                ))}
                <th className="px-5 py-3.5 text-right text-xs font-bold uppercase tracking-wide">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                [...Array(6)].map((_, i) => (
                  <tr key={i}>
                    {[...Array(8)].map((_, j) => (
                      <td key={j} className="px-5 py-4"><div className="h-4 w-24 animate-pulse rounded bg-slate-200"></div></td>
                    ))}
                  </tr>
                ))
              ) : employees.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-5 py-16 text-center">
                    ...
                    <p className="mt-3 font-semibold text-slate-700">No employees found</p>
                    <p className="mt-1 text-sm text-slate-500">Try adjusting your filters or add a new employee.</p>
                  </td>
                </tr>
              ) : (
                employees.map(emp => (
                  <tr key={emp.id} className="transition hover:bg-emerald-50/35">
                    ...
                    <td className="px-5 py-4">
                      <span className="rounded-md bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{emp.department}</span>
                    </td>
                    <td className="px-5 py-4 text-slate-700">{emp.country}</td>
                    <td className="px-5 py-4 font-semibold text-emerald-700">{formatCurrency(emp.salary)}</td>
                    <td className="px-5 py-4">
                      <span className={`rounded-md px-2.5 py-1 text-xs font-semibold ${
                        emp.employment_type === 'Full-time' ? 'bg-emerald-50 text-emerald-700' :
                        emp.employment_type === 'Contract' ? 'bg-amber-50 text-amber-700' :
